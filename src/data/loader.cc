...
std::unordered_map<TKey, TParseable *>
ParseJsons(std::ifstream stream, std::function<TParseable *()> fac,
           std::function<TKey(TParseable *)> key_getter) {
  nlohmann::json json;
  stream >> json;
...
void DataRegistry::Load() {
  ...
  // interactable
  this->interact_by_char_ = ParseJsons<char, Interactable>(
      std::move(interact), []() { return new Interactable(); },
      [](auto i) { return i->map_char_; });
  this->effects_ = ParseJsons<std::string, Effect>(
      std::move(effects), []() { return new Effect(); },
      [](auto o) { return o->id_; });
  this->mobs_ = ParseJsons<std::string, Mob>(
      std::move(mobs), []() { return new Mob(); },
      [](auto o) { return o->id_; });
  this->items_ = ParseJsons<std::string, Item>(
      std::move(items), []() { return new Item(); },
      [](auto o) { return o->id_; });
}