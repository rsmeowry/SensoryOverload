...
bool AudioSystem::Init() {
  ...
  if (alcIsExtensionPresent(device_, "ALC_EXT_EFX")) {
    al_gen_filters =
        reinterpret_cast<LPALGENFILTERS>(alGetProcAddress("alGenFilters"));
    al_delete_filters = reinterpret_cast<LPALDELETEFILTERS>(
    ...
    exf_available =
        (al_gen_filters && al_delete_filters && al_filter_i && al_filter_f);
  }
