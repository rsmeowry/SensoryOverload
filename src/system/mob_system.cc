...
void MobSystem::Update(MapData &map, GlobalState &state,
  ...
  for (auto &mob : map.mobs_) {
    ...
    for (const auto &n : neighbors) {
      if (n.x >= 0 && n.y >= 0 && n.x < map.size_ && n.y < map.size_) {
        bool is_player = (n.x == state.player_.transform_.x_ &&
                          n.y == state.player_.transform_.y_);

        auto obj = map.ObjAt(n.x, n.y);
        if (is_player || (obj && !obj->solid_)) {
          if (std::max(std::abs(n.x - origin_x), std::abs(n.y - origin_y)) <=
              radius) {
    ...
    if (!valid_moves.empty()) {
      ...
      if (next_move.x == state.player_.transform_.x_ &&
          next_move.y == state.player_.transform_.y_) {
        ...
        RenderSystem::PrintMessage(
            "you feel a burst of pain from somewhere. you took " +
